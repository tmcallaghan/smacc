...
async function runTests() {
  ...
  try {
    ...
    await test('$addToSetWindow ($setWindowFields)', () => coll.aggregate([{$setWindowFields: {sortBy: { age: 1 },output: {testList: {$addToSet: "$dept",window: {documents: ["unbounded", "current"]}}}}}]).toArray())
    ...
    await test('$denseRank ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $denseRank: {} } } } }]).toArray());
    await test('$derivative ($setWindowFields)', () => coll.aggregate([{$setWindowFields: {sortBy: { age: 1 },output: {salaryChange: {$derivative: {input: "$salary"}, window: {documents: ["unbounded", "unbounded"]}}}}}]).toArray())
    await test('$documentNumber ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $documentNumber: {} } } } }]).toArray());
    await test('$expMovingAvg ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $expMovingAvg: { input: '$score', N: 2 } } } } }]).toArray());
    await test('$firstWindow ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $first: '$name' } } } }]).toArray());
    await test('$firstNWindow ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $firstN: { input: '$name', n: 2 } } } } }]).toArray());
    ...
    await test('$sumWindow ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $sum: '$score' } } } }]).toArray());
    await test('$topWindow ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $top: { output: '$name', sortBy: { score: -1 } } } } } }]).toArray());
    await test('$topNWindow ($setWindowFields)', () => coll.aggregate([{ $setWindowFields: { sortBy: { age: 1 }, output: { result: { $topN: { output: '$name', sortBy: { score: -1 }, n: 2 } } } } }]).toArray());

    // OPERATORS - Variable
    await test('$let', () => coll.aggregate([{ $project: { result: { $let: { vars: { x: 5 }, in: { $multiply: ['$$x', 2] } } } } }]).toArray());

    // OPERATORS - Query and Projection (in $match)
    await test('$all ($match)', () => coll.aggregate([{ $match: { tags: { $all: ['a', 'b'] } } }]).toArray());
    await test('$andQuery ($match)', () => coll.aggregate([{ $match: { $and: [{ age: { $gt: 25 } }, { score: { $gt: 80 } }] } }]).toArray());
    await test('$bitsAllClear ($match)', () => coll.aggregate([{ $match: { age: { $bitsAllClear: [1, 5] } } }]).toArray());
    await test('$bitsAllSet ($match)', () => coll.aggregate([{ $match: { age: { $bitsAllSet: [1, 5] } } }]).toArray());
    await test('$bitsAnyClear ($match)', () => coll.aggregate([{ $match: { age: { $bitsAnyClear: [1, 5] } } }]).toArray());
    ...
    await test('$orQuery ($match)', () => coll.aggregate([{ $match: { $or: [{ age: { $lt: 26 } }, { age: { $gt: 33 } }] } }]).toArray());
    await test('$regexQuery ($match)', () => coll.aggregate([{ $match: { name: { $regex: /^A/ } } }]).toArray());
    await test('$sampleRate ($match)', () => coll.aggregate([{ $match: { $sampleRate: 0.5 } }]).toArray());
    await test('$sizeQuery ($match)', () => coll.aggregate([{ $match: { tags: { $size: 2 } } }]).toArray());
    await test('$text ($match)', () => Promise.resolve([]), true);
    await test('$typeQuery ($match)', () => coll.aggregate([{ $match: { name: { $type: 'string' } } }]).toArray());
